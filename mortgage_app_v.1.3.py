...
if visible_opts:
    ...
    for i, opt in enumerate(visible_opts):
        with tabs[i]:
            ...
            # Детализация платежа (сворачиваемый блок)
            with st.expander("💳 Разбивка платежа за первый месяц"):
                total_first = res['first'] + (ren['monthly'] if ren else 0)
                st.markdown(f"**Общая сумма: {format_money(total_first)}**")
                lines_text = ""
            ...
            # Горизонтальная линия необходимого дохода
            ...
            # График №2: структура платежа (шкала в годах)
            st.subheader("🍰 Структура ежемесячного платежа (по годам)")
            num_tranches = len(opt.tranches)
            stacked_data = {f"Линия {j+1} ({t.rate}%)": [] for j, t in enumerate(opt.tranches)}
            if ren:
                stacked_data["Ремонт"] = []

            for m_idx in range(actual_months):
    ...
    with tabs[-1]:
        ...
        for opt in visible_opts:
            ...
            # Разбивка по линиям и ремонту (среднемесячные)
            # Считаем среднемесячные для каждой линии и ремонта за весь срок
            avg_line_costs = []
            for j, t in enumerate(opt.tranches):
                line_total = sum(res['payments_by_tranche'][m][j] for m in range(actual_months) if m < len(res['payments_by_tranche']))
            ...
            # Собираем данные для stacked bar: снизу вверх линии, потом ремонт
            y_base = 0
            for j, t in enumerate(opt.tranches):
        ...
        fig_bar_avg.update_layout(
            barmode='stack',
            yaxis_title="Среднемесячная сумма, ₽",
            template='plotly_white',
            legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1)
        )
        ...
        for opt in visible_opts:
            ...
            # Горизонтальная линия необходимого годового дохода для этого варианта
            total_cost_all = res['total_paid'] + (opt.renovation_total_cost if opt.renovation_total_cost else 0)
            avg_all = total_cost_all / actual_months if actual_months > 0 else 0
            ...
            fig_yearly.add_hline(
                ...
                line_dash="dash",
                line_color="gray",
                annotation_text=f"{opt.name}: {format_money(annual_req)}/год",
                annotation_position="top right"
            )
        fig_yearly.update_layout(
            xaxis_title="Год",
            yaxis_title="Общие расходы за год, ₽",
            template='plotly_white',
            hovermode='x unified'
        )
