...
with st.sidebar:
    ...
    with st.form("add_option"):
        ...
        if total_loan <= 0:
            ...
            for i in range(int(lines_count)):
                ...
                with cols[0]:
                    # ИСПРАВЛЕНИЕ: явное приведение к float для совместимости типов
                    if i == 0:
    ...
    if submitted and total_loan > 0:
        ...
        for amt, rt in tranches:
            if amt > 0:
                final_tranches.append(Tranche(float(amt), float(rt)))
        if final_tranches:
            opt = MortgageOption(
                name=name,
...
if visible_opts:
    ...
    for i, opt in enumerate(visible_opts):
        with tabs[i]:
            ...
            # График платежей – линии с точками, без заливки
