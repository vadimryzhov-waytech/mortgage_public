...
with st.sidebar:
    ...
    # Управление вариантами: удаление (ЗАМЕТНЫЕ КНОПКИ)
    if st.session_state.options:
        st.divider()
        st.subheader("🗑️ Управление вариантами")
        for i, opt in enumerate(st.session_state.options):
            col_opt, col_btn = st.columns([3, 1])
            with col_opt:
                st.write(f"**{opt.name}**")
            with col_btn:
                if st.button("🗑️ Удалить", key=f"del_{i}", type="primary", use_container_width=True):
                    del st.session_state.options[i]
                    st.rerun()

# ------------------- Основная область: отображение -------------------
visible_opts = st.session_state.options  # показываем все оставшиеся
...
if visible_opts:
    ...
    st.info("👈 Добавьте первый вариант ипотеки через боковую панель слева.")